...
namespace Rogue.NET.Scenario.Content.ViewModel.ItemGrid
{
    ...
    [Export]
    public class EquipmentIdentifyItemGridViewModel : EquipmentItemGridViewModelBase
    {
        [ImportingConstructor]
        public EquipmentIdentifyItemGridViewModel(
        ...
        protected override bool GetIsEnabled(IModelService modelService, Equipment item)
        {
            return !modelService.ScenarioEncyclopedia[item.RogueName].IsIdentified ||
                  (!item.IsIdentified && item.ClassApplies());
        }
    }
}
